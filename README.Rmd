...
2. Extract data from the R10s (L. Harrison helped with this) and correct raw data with an OPUS correction.
...
8. Convert TIN to raster layer (in ArcMap).

9. TBD - Compair WY22 layer to ESA Dec 2016 layer. Raster or TIN differencing (in ArcMap? or R?). 
...
* Survey extent: Scott Creek State Beach inland to Queseria Creek confluence. Most topo points are from the beach to the north marsh area (bad signal starting around the Lagoon PIT antenna array). Echosounder points focused on the main channel from the beach (downstream side of Hwy 1 bridge) to Queseria Creek confluence. 
...
# 2. OPUS Correction
 ...
 (raw base station file submitted to OPUS and then apply correction to all points)
